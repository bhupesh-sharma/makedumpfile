 ...
 * only on ARMv8.2 arch variants and above, so we use
 * the gcc construct '__attribute__ ((target ("arch=armv8.2-a")))'
 * here which is a arch extension (see AArch64-Target-selection-directives
 * documentation from ARM) just for this function to make
 * sure it compiles well with binutils/gcc.
 */
...
get_page_offset_arm64(void)
{
	...
	if (!vabits_actual) {
		...
		if (vabits_actual == ERROR) {
			/* If we cannot read ID_AA64MMFR2_EL1 arch
			 * register, our last option is to use VA_BITS
			 * to calculate the PAGE_OFFSET value.
			 *
			 * See arch/arm64/include/asm/memory.h for more
			 * details of the PAGE_OFFSET calculation.
			 */
			info->page_offset = (-(1UL << va_bits));
			DEBUG_MSG("page_offset    : %lx (via va_bits)\n",
					info->page_offset);
			return;
		} else
